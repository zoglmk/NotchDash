...
- **Claude Code / Codex 额度**：剩余或已用百分比、重置倒计时，按状态变色；额度见底时改显示"还有多久恢复"
- **系统状态**：CPU、内存、实时网速
- **股票 / 指数**：A 股、港股、美股，按涨跌上色
- **轮播**：收起态轮流显示额度与行情，黑条宽度不变
- **自动避让**：实测菜单栏图标位置，自动挑一种不打架的排布
- **自定义数据源**：任意 shell 命令的输出都能显示
...
到 [Releases](../../releases) 下载 zip，解压后把 `NotchDash.app` 拖进「应用程序」。

**首次打开会被系统拦下**，提示「已损坏，无法打开，你应该将它移到废纸篓」。
这个提示是误导性的——文件没坏，只是本项目没有 Apple 签名和公证
（需要 99 美元/年的开发者账号）。任选一种方式解除：
...
这条命令只是移除 macOS 给「从网上下载的文件」打的隔离标记，之后双击即可正常打开。

**方式二：系统设置**

1. 双击 `NotchDash.app`，出现拦截提示后点「完成」
2. 打开「系统设置 → 隐私与安全性」，往下滚
3. 「安全性」一节会出现「已阻止使用 NotchDash」，点右侧「仍要打开」
4. 再确认一次即可

> macOS 15 之后，右键图标选「打开」的老办法**已经失效**，只能用上面两种。

从源码 `./install.sh` 构建则完全不受影响——本地编译的产物没有隔离标记。
...
收起态只有两个位置，开了轮播就能把额度和行情轮流显示：

```
CC 28%  Codex 65%   →   上证 3912 +0.94%   →   纳指 26523 +0.39%   →  回到额度
```

菜单里点「轮播显示额度与行情」开关，`carousel_interval` 控制间隔（秒，最小 2）。

两条设计约束：

- **行情一次只显示一个**。两个并排会把黑条撑长，而单个（含点位）比额度页那两项还窄。
- **面板宽度锁定在额度页**，不跟着当前页走，也不取各页最大值——轮播时黑条必须
  纹丝不动，一胀一缩比不轮播还晃眼。
...
读状态栏图标的坐标要用辅助功能接口，所以得授权一次：

1. 首次启动时会弹系统授权请求，点「打开系统设置」
2. 或手动打开「系统设置 → 隐私与安全性 → 辅助功能」
3. 在列表里找到 **NotchDash**，打开右侧开关
4. 不用重启，20 秒内自动生效

**拒绝也完全能用**，只是排布得自己在 `⋯` 菜单 →「收起态排布」里挑一种。

什么时候要重新授权：

- **更新应用之后**：macOS 按签名认应用，换掉 `.app` 后授权可能失效。
  表现是自动排布不再生效——把列表里的 NotchDash 删掉（选中点 `−`）再重新添加
- **列表里出现两个 NotchDash**：删掉旧的那个

想确认当前有没有拿到权限，看 `~/.notchdash/status.json` 里的
`accessibility_authorized`，或者看菜单里「收起态排布 → 自动」是否显示为
「需辅助功能权限」。
...
- `carousel` / `carousel_interval`：收起态轮播开关与间隔（秒，最小 2）
- `red_up`：`true` 红涨绿跌，`false` 绿涨红跌
...
**在菜单里点就行**，不用改配置：面板上右键（或展开后点 `⋯`）→「行情」，
常用指数直接勾选，个股用「添加代码…」输入，名称会自动获取；
「刷新频率」可选 10 秒 / 30 秒 / 1 分钟 / 5 分钟。改完立即生效。

预设有：上证指数、深证成指、创业板指、沪深300、恒生指数、道琼斯、纳斯达克、标普500。

也可以直接改配置文件（数据来自新浪财经公开接口，不需要 API Key）：
...
显示形如 `3912 +0.94%`，按涨跌上色（`red_up` 控制红涨绿跌还是绿涨红跌）。
多个代码会合并成一次请求。常用代码：
...
| `s_sz399006` | 创业板指 | `gb_inx` | 标普500 |

个股直接写代码，如 `sh600519`（贵州茅台）、`sz000001`（平安银行）。

关于实时性：交易时段内是行情快照，免费接口通常有秒级到分钟级延迟；
加上 `interval` 的轮询间隔，属于「准实时」。非交易时段显示最近一次收盘数据，
不会变化。
