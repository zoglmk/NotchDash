...
自带一个取行情的脚本（新浪财经公开接口，不需要 API Key）：

```json
"custom_sources": [
  { "label": "上证", "command": "~/minitool/scripts/quote.sh s_sh000001", "interval": 60 },
  { "label": "纳指", "command": "~/minitool/scripts/quote.sh gb_ixic", "interval": 60 }
]
```
...
个股直接用代码，如 `sh600519`（贵州茅台）、`sz000001`（平安银行）。
非交易时段返回最近一次收盘数据。
